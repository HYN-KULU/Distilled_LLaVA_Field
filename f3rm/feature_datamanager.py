...
class FeatureDataManager(VanillaDataManager):
    ...
    def __init__(self, *args, **kwargs):
        ...
        print("Check the metadata: ","feature_type: ",self.train_dataset.metadata["feature_type"], "feature_dim", self.train_dataset.metadata["feature_dim"])
        ...
        print("Camera Image Height & Width: ",im_h,im_w)
        print("CLIP Feature Map Height & Width: ",feat_h,feat_w)
        print("Height & Width Scale: ",self.scale_h,self.scale_w)
        assert np.isclose(
    ...
    ### In this function, process the images to CLIP embeddings in patch level.
    def extract_features(self) -> Float[torch.Tensor, "n h w c"]:
        """Extract features with support for caching."""
        print("Feature Type: ",self.config.feature_type)
        ...
        if self.config.enable_cache and cache_path.exists():
            print("Extracted Feature Cache: ",cache_path)
            print("I use the Cache.")
