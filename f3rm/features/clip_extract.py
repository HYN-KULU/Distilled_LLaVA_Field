...
def extract_clip_features(image_paths: List[str], device: torch.device) -> torch.Tensor:
    ...
    for i in tqdm(
        ...
        print("Image batch shape: ",batch.shape)
        embedding=model.get_patch_encodings(batch)
        print("Patch Encoding shape: ",embedding.shape)
        embeddings.append(embedding)
    embeddings = torch.cat(embeddings, dim=0)

    # Reshape embeddings from flattened patches to patch height and width
    h_in, w_in = preprocessed_images.shape[-2:]
    print("h_in, w_in, model.visual.patch_size: ",h_in,w_in,model.visual.patch_size)
